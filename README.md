...
1. 在 '按住时重复' 的宏动作，如果延迟不够，触发过于频繁，可能会导致在松开触发键时，宏动作不会及时停下。

> 解决方案：
   方案A. 快速按下其他触发键来执行其他宏动作，这样就会重置了。
   方案B. 快速按下两次 ESC 键，来强制停止所有宏动作。
   方案C. 按下 F10 / F11 / F12 任意进程键，来停止 / 重载配置 / 退出宏脚本。
   方案D. 切换游戏窗口，检测到窗口和游戏不一致时，脚本会立即停止。
